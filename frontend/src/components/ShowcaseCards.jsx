    ...
    <div className="relative w-full max-w-6xl mx-auto">
      <div className="relative flex items-end justify-center gap-6 md:gap-10 py-10">
        {CARDS.map((card, index) => {
          ...
          const sideTint = isActive
            ? ""
            : "after:absolute after:inset-0 after:bg-gradient-to-b after:from-black/30 after:to-black/10";
          ...
          return (
            ...
            >
              ...
              {/* Center glow */}
              {isActive && (
                <div className="absolute inset-0 bg-gradient-to-b from-white/10 via-white/0 to-white/20" />
              )}

              {/* Title + conditional description */}
              <div
                className={`absolute left-0 right-0 -bottom-2 md:bottom-0 mx-auto ${
                  isActive ? "w-[80%]" : "w-[90%]"
                } bg-white/70 backdrop-blur-md rounded-[28px] p-4 md:p-6 text-center ring-1 ring-black/10`}
              >
                <h4
      ...
      <div className="flex items-center justify-center gap-4 mt-2">
        <button
          onClick={prev}
          className="px-3 py-2 rounded-full bg-black/10 hover:bg-black/20 transition"
        >
          Prev
        </button>
          ...
          {CARDS.map((_, i) => (
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full ${
                i === active ? "bg-emerald-600" : "bg-emerald-300"
              }`}
            />
          ))}
        </div>
